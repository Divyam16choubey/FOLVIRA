    ...
    <section id="templates" className="border-y border-line bg-[#eeebe3] py-20 sm:py-28">
      <div className="section-shell">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {templates.map((template, index) => <Reveal key={template.name} delay={index * 0.07}><article className="group"><TemplateFrame kind={template.kind} /><div className="mt-4 flex items-start justify-between gap-3"><div><h3 className="text-base font-extrabold tracking-[-0.025em]">{template.name}</h3><p className="mt-1 text-sm leading-5 text-muted">{template.description}</p></div><span className="pt-0.5 text-brass transition-transform duration-200 group-hover:translate-x-0.5">↗</span></div></article></Reveal>)}
        </div>
      </div>
    </section>
  )
...
function TemplateFrame({ kind }: { kind: 'editorial' | 'minimal' | 'developer' | 'creative' }) {
  return <div className={`template-frame template-${kind}`} aria-label={`${kind} portfolio template preview`}>
    ...
    {kind === 'developer' && <><div className="template-dev-code">const work = <b>‘useful’</b>;<br /><span>// Farah A.</span></div><div className="template-dev-window"><i /><i /><i /></div></>}
    {kind === 'creative' && <><div className="template-creative-block template-creative-one" /><div className="template-creative-block template-creative-two" /><div className="template-creative-copy">MAKE<br />THINGS<br /><b>MOVE.</b></div></>}
  </div>
}
