...
export function PortfolioPreview() {
  ...
  return (
    <motion.div initial={reducedMotion ? false : { opacity: 0, scale: 0.97, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.72, delay: 0.18, ease: [0.22, 1, 0.36, 1] }} className="relative mx-auto w-full max-w-[770px] lg:mx-0">
      ...
      <article className="overflow-hidden rounded-card border border-ink/10 bg-paper shadow-lift">
        ...
        <div className="relative grid min-h-[370px] overflow-hidden px-5 py-8 sm:min-h-[418px] sm:grid-cols-[1fr_0.85fr] sm:px-7 sm:py-11">
          <div className="relative z-10 flex flex-col items-start">
            ...
            <button className="mt-5 inline-flex items-center gap-2 rounded-md bg-pine px-3.5 py-2.5 text-[10px] font-extrabold text-white transition-colors hover:bg-[#183d35] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-pine">View selected work <ArrowUpRightIcon size={13} /></button>
            <div className="mt-auto hidden gap-2.5 pt-5 sm:flex" aria-label="Demo social links">
